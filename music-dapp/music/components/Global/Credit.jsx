...
const Credit = ({ setCredit, activeUser, setLoader }) => {
  ...
  ///VERIFY_ACCOUNT
  const VERIFY_ACCOUNT = async (VERIFY_AMOUNT, OWNER_ADDRESS) => {
    try {
      setLoader(true);
      ...
      const parsedAmount = ethers.utils.parseEther(VERIFY_AMOUNT.toString());

      console.log(accounts[0]);

      const transition = await ethereum.request({
  ...
  ///VERIFY_ACCOUNT
  const BUY_CREDIT = async (CREDIT_AMOUNT, OWNER_ADDRESS) => {
    try {
      setLoader(true);
      const currentUser = await CHECK_AUTH();
      ...
      const parsedAmount = ethers.utils.parseEther(CREDIT_AMOUNT.toString());
                      ...
                      <div className="block rounded-md bg-white hover:bg-base50">
                        <div className="flex flex-col gap-1 p-3">
                          ...
                          <div className="mt-2 flex w-full gap-2 [&_a]:w-full">
                            <button
                              onClick={() =>
                                VERIFY_ACCOUNT(VERIFY_AMOUNT, OWNER_ADDRESS)
                              }
